...
/* kvs_vector_t 类型在 kvs_vector.c 中定义，前向声明 */
#if ENABLE_VECTOR
struct kvs_vector_internal;
typedef struct kvs_vector_internal kvs_vector_t;

/* 全局向量实例大小（供 kvstore.c 中 memset 使用） */
/* 注意: 由于 kvs_vector_t 是前向声明，无法在此处使用 sizeof */
/* 使用 0 初始化 + kvs_vector_create() 来替代 memset */
...
int kvs_vector_search(kvs_vector_t *vec, char *key, int k, char *query_str, int *result_ids, float *result_dists);
